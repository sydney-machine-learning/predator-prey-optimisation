...
% Objective Function
F_index=14;
ObjectiveFunction=@test_functions;
[down,up]=test_functions_range(F_index);
ProblemSize=[1 100];         % Decision Variables Size
LowerBound=  down;             % Decision Variables Lower Bound
UpperBound=  up;             % Decision Variables Upper Bound
%% Mayfly Parameters
methname='Mayfly Algorithm';
MaxIt=1000;                 % Maximum Number of Iterations
nPop=15; nPopf=15;          % Population Size (males and females)
...
fl_damp=0.99;
% Mating Parameters
nc=16;                      % Number of Offsprings (also Parnets)
...
for i=1:nPop
    ...
    % Evaluation
    Mayfly(i).Cost=ObjectiveFunction(Mayfly(i).Position,F_index);
...
for i=1:nPopf
    ...
    Mayflyf(i).Cost=ObjectiveFunction(Mayflyf(i).Position,F_index);
    funccount=funccount+1;
...
for it=1:MaxIt
    ...
    for k=1:nc/2
        ...
        % Evaluate Offsprings
        MayflyOffspring(k,1).Cost=ObjectiveFunction(MayflyOffspring(k,1).Position,F_index);
        if MayflyOffspring(k,1).Cost<GlobalBest.Cost
            GlobalBest=MayflyOffspring(k,1);
        end
        funccount=funccount+1;
        MayflyOffspring(k,2).Cost=ObjectiveFunction(MayflyOffspring(k,2).Position,F_index);
    ...
    for k=1:nm
        ...
        % Evaluate Mutant
        MutMayflies(k).Cost=ObjectiveFunction(MutMayflies(k).Position,F_index);
